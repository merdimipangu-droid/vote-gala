...
// ========== FONCTIONS DE VOTE AVEC SUPABASE ==========
...
async function saveUserVote(categorie, categorie_nom, candidat_nom) {
...
async function hasUserVotedInCategory(categorie) {
...
async function markUserVotedInCategory(categorie) {
    if (!currentUser) return false;
    const { error } = await supabase
        .from('votants')
        .insert([{
...
async function addComment(name, message) {
    ...
    const newComment = { user_id: currentUser?.id, user_email: currentUser?.email, nom: name.trim() || "Anonyme", message: message.trim(), created_at: new Date().toISOString() };
...
async function displayComments() {
    ...
    container.innerHTML = '';
    comments.forEach(comment => { const commentDate = new Date(comment.created_at); const dateStr = commentDate.toLocaleString('fr-FR'); const card = document.createElement('div'); card.className = 'comment-card'; card.innerHTML = `<div class="comment-header"><div class="comment-author"><i class="fas fa-user-circle"></i><strong>${escapeHtml(comment.nom || comment.user_email || 'Anonyme')}</strong></div><div class="comment-date"><i class="far fa-calendar-alt"></i> ${dateStr}</div></div><div class="comment-message">${escapeHtml(comment.message)}</div>`; container.appendChild(card); });
}
...
async function chargerAdminComments() {
    ...
    container.innerHTML = '';
    comments.forEach(comment => { const date = new Date(comment.created_at); const card = document.createElement('div'); card.className = 'admin-comment-card'; card.innerHTML = `<div class="admin-comment-header"><strong><i class="fas fa-user"></i> ${escapeHtml(comment.nom || comment.user_email || 'Anonyme')}</strong><span>${date.toLocaleString('fr-FR')}</span></div><div class="admin-comment-message">${escapeHtml(comment.message)}</div><div class="admin-comment-ip">ID: ${comment.user_id || 'N/A'}</div><button class="admin-comment-delete-btn" data-id="${comment.id}"><i class="fas fa-trash-alt"></i> Supprimer</button>`; container.appendChild(card); const deleteBtn = card.querySelector('.admin-comment-delete-btn'); if (deleteBtn) deleteBtn.onclick = () => deleteComment(comment.id, true); });
}
...
async function voter(categorie, candidatNom) {
    ...
    const hasVoted = await hasUserVotedInCategory(categorie);
    if (hasVoted) { 
    ...
    const voteSaved = await saveUserVote(categorie, catName, candidatNom);
    if (voteSaved) {
        await markUserVotedInCategory(categorie);
...
async function afficherDashboard() {
    ...
    if (!currentUser) {
        dashboardDiv.innerHTML = '<div class="loading-results"><i class="fas fa-spinner fa-pulse"></i> Veuillez vous connecter pour voir vos votes.</div>';
        return;
    }
    ...
    for (const [catId, data] of Object.entries(resultatsParCategorie)) {
        ...
        if (data.aVote) {
            card.innerHTML = `<h3><i class="fas fa-check-circle"></i> ${data.nom}</h3><div class="my-vote"><i class="fas fa-vote-yea"></i> Votre vote : <strong>${data.monVote}</strong></div><div class="vote-message">Merci pour votre participation !</div>`;
        } else {
            card.innerHTML = `<h3><i class="far fa-circle"></i> ${data.nom}</h3><div class="my-vote"><i class="fas fa-clock"></i> En attente de votre vote</div><div class="vote-message">Retournez voter pour cette catégorie</div>`;
        }
...
async function afficherResultatsParCategorie(categoryId) {
    ...
    const { data: votesForCategory, error } = await supabase
        .from('votes')
        .select('*')
    ...
    votesForCategory.forEach(vote => { votesByCandidate[vote.candidat_nom] = (votesByCandidate[vote.candidat_nom] || 0) + 1; });
...
async function afficherTousResultatsFinaux() {
    ...
    allVotes.forEach(vote => { if (resultats[vote.categorie]) { resultats[vote.categorie].votes[vote.candidat_nom] = (resultats[vote.categorie].votes[vote.candidat_nom] || 0) + 1; resultats[vote.categorie].total++; } });
...
async function chargerAdminData() { 
    const { data: allVotes, error: votesError } = await supabase.from('votes').select('*');
    const { data: allVotants, error: votantsError } = await supabase.from('votants').select('*');
    if (votesError || votantsError) return;
    const totalVotes = allVotes.length;
    const uniqueUsers = new Set(allVotes.map(v => v.user_id)).size;
    ...
    const statsGrid = document.getElementById('statsGrid'); if(statsGrid) statsGrid.innerHTML = `<div class="stat-card"><div class="stat-number">${totalVotes}</div><div class="stat-label">Votes totaux</div></div><div class="stat-card"><div class="stat-number">${uniqueUsers}</div><div class="stat-label">Votants uniques</div></div><div class="stat-card"><div class="stat-number">${categories.length}</div><div class="stat-label">Catégories</div></div><div class="stat-card"><div class="stat-number">${tauxParticipation}%</div><div class="stat-label">Objectif atteint</div></div><div class="stat-card"><div class="stat-number">${votesRestants}</div><div class="stat-label">Votes restants</div></div><div class="stat-card"><div class="stat-number">${MAX_VOTES}</div><div class="stat-label">Objectif max</div></div>`;
    const container = document.getElementById('categorySummary'); if(container) { const resultatsParCategorie = {}; categories.forEach(cat => { resultatsParCategorie[cat.id] = { nom: cat.name, votes: {}, total: 0 }; }); allVotes.forEach(vote => { if(resultatsParCategorie[vote.categorie]) { resultatsParCategorie[vote.categorie].votes[vote.candidat_nom] = (resultatsParCategorie[vote.categorie].votes[vote.candidat_nom] || 0) + 1; resultatsParCategorie[vote.categorie].total++; } }); container.innerHTML = ''; for (const [catId, data] of Object.entries(resultatsParCategorie)) { const sorted = Object.entries(data.votes).sort((a, b) => b[1] - a[1]); const winner = sorted[0]; const catIcon = categories.find(c => c.id === catId)?.icon || ''; const card = document.createElement('div'); card.className = 'category-summary-card'; card.innerHTML = `<h4>${catIcon} ${data.nom}</h4><div class="total"><i class="fas fa-chart-simple"></i> Total votes : ${data.total}</div><div class="winner"><i class="fas fa-crown"></i> Leader : ${winner ? winner[0] : 'Aucun vote'} (${winner ? winner[1] : 0} voix)</div>`; container.appendChild(card); } }
    const tbody = document.getElementById('votesTableBody'); if(tbody) { if(allVotes.length === 0) { tbody.innerHTML = '<tr><td colspan="4" class="loading-cell">Aucun vote enregistré</td></tr>'; } else { tbody.innerHTML = ''; [...allVotes].reverse().forEach(vote => { const date = new Date(vote.created_at); tbody.innerHTML += `<tr><td>${date.toLocaleString('fr-FR')}</td><td><strong>${vote.categorie_nom}</strong></td><td>${vote.candidat_nom}</td><td><code>${vote.user_id?.substring(0, 8)}...</code></td></tr>`; }); applyTableFilters(); } } }

function exporterDonneesAdmin() { 
    supabase.from('votes').select('*').then(({ data: votes }) => {
        supabase.from('votants').select('*').then(({ data: votants }) => {
            const data = { exportDate: new Date().toISOString(), evenement: "Gala 25 ans", statistiques: { totalVotes: votes.length, totalVotants: new Set(votes.map(v => v.user_id)).size, categories: categories.length, voteStatus: localStorage.getItem('voteStatus'), maxVotes: MAX_VOTES }, votes: votes, votants: votants, commentaires: comments, publishedCategories: publishedCategories, allResultsPublished: allResultsPublished }; 
            const blob = new Blob([JSON.stringify(data, null, 2)], {type: 'application/json'}); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `votes_gala_${new Date().toISOString().slice(0,19)}.json`; a.click(); URL.revokeObjectURL(a.href); showToast("Export JSON généré !", "success");
        });
    });
}
...
document.getElementById('refreshBtn')?.addEventListener('click', () => { chargerAdminData(); initVotesChart(); renderPublishCategoriesGrid(); chargerAdminComments(); showToast("Données rafraîchies !", "info"); });
...
document.getElementById('resetAllBtn')?.addEventListener('click', async () => { if(confirm("⚠️ RÉINITIALISATION COMPLÈTE ?\nCette action est IRRÉVERSIBLE !")) { await supabase.from('votes').delete().neq('id', 0); await supabase.from('votants').delete().neq('id', 0); await supabase.from('commentaires').delete().neq('id', 0); currentUserVotes.clear(); publishedCategories = []; allResultsPublished = false; savePublishedCategories(); localStorage.removeItem('visitor_ip'); localStorage.removeItem('adminLoggedIn'); localStorage.removeItem('resultsPublished'); localStorage.removeItem('resultsDisplayed'); localStorage.removeItem('timerEndTime'); localStorage.removeItem('voteStatus'); clearDisplayedCategories(); setVoteStatus('open'); localStorage.setItem('resultsPublished', 'false'); chargerAdminData(); chargerAdminComments(); if (votesChart) votesChart.destroy(); initVotesChart(); renderPublishCategoriesGrid(); showToast("✅ Toutes les données ont été réinitialisées !", "success"); } }); }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (userVotes) {
        userVotes.forEach(vote => {
            currentUserVotes.add(vote.categorie);
        });
    }
