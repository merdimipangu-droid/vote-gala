...
function chargerVotesExistants() {
    ...
    for (const [voterIp, categoriesVoted] of Object.entries(localVotants)) { 
        if (voterIp === ip) { 
            Object.keys(categoriesVoted).forEach(catId => { currentUserVotes.add(catId); }); 
        } 
    }
}
...
function addComment(name, message) {
    ...
    const newComment = { id: Date.now(), nom: name.trim() || "Anonyme", message: message.trim(), ip: getIP(), created_at: new Date().toISOString() };
...
function initComments() {
    ...
    submitBtn.onclick = () => { const name = commentName ? commentName.value : ''; const message = commentMessage ? commentMessage.value : ''; if (addComment(name, message)) { if (commentMessage) commentMessage.value = ''; if (charCount) charCount.textContent = '0'; } };
    displayComments();
}
...
function voter(categorie, candidatNom) {
    ...
    localVotes.push({ categorie: categorie, categorie_nom: catName, candidat_nom: candidatNom, created_at: new Date().toISOString(), ip: ip });
...
document.getElementById('refreshBtn')?.addEventListener('click', () => { loadData(); chargerAdminData(); initVotesChart(); renderPublishCategoriesGrid(); chargerAdminComments(); showToast("Données rafraîchies !", "info"); });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (path.includes('resultat.html')) { initResultsPage(); addForceRefreshButton(); }
    else if (path.includes('admin.html')) { initAdmin(); }
    else { chargerVotesExistants(); afficherCategories(); afficherDashboard(); startRealtimeResultsListener(); }
});
