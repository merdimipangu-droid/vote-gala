...
const candidats = {
    ...
    Ponctualite: [
        ...
        { id: "pon3", nom: "Djessy", description: "30 minutes minimum, désorganisée mais attachante", photo: "assets/Ponctualite/djessy1.jpeg" },
        { id: "pon4", nom: "Keren", description: "toujours pile à l'heure, pas une minute avant.", photo: "assets/Ponctualite/keren1.jpeg" },
        { id: "pon5", nom: "Emilie", description: "5 à 15 minutes, avec une bonne excuse", photo: "assets/Ponctualite/emilie1.jpeg" }
    ],
    Meilleur_Soliste: [
...
function showVoteClosedWarning() {
    if (activeWarningToast) {
        activeWarningToast.remove();
        activeWarningToast = null;
    }
    ...
    toast.innerHTML = `
        <div class="toast-warning-content">
            ...
            <div class="toast-warning-message">
                <strong>🔒 Votes clôturés</strong>
                <p>Les votes sont terminés.<br>Résultats disponibles bientôt.</p>
            </div>
        </div>
        <div class="toast-warning-progress">
            <div class="toast-warning-progress-bar"></div>
        </div>
    `;
    ...
    setTimeout(() => {
        if (activeWarningToast) {
            activeWarningToast.classList.remove('show');
            setTimeout(() => {
                if (activeWarningToast) {
                    activeWarningToast.remove();
                    activeWarningToast = null;
...
function showVoteClosedBanner() {
    ...
    banner.innerHTML = `
        <div class="vote-closed-banner-content">
            <div class="vote-closed-message">
                <div class="vote-closed-icon"><i class="fas fa-bell"></i></div>
                <div class="vote-closed-text">
                    <h3><i class="fas fa-lock"></i> VOTES TERMINÉS</h3>
                    <p>Les votes sont officiellement clôturés. Les résultats seront annoncés prochainement.</p>
                </div>
            </div>
            <div class="vote-closed-badge">
                <i class="fas fa-hourglass-half"></i> Fin du vote
            </div>
        </div>
    `;
...
function hideVoteClosedBanner() {
    if (voteClosedBanner) {
        voteClosedBanner.classList.remove('show');
        setTimeout(() => { if (voteClosedBanner) voteClosedBanner.remove(); voteClosedBanner = null; }, 500);
    }
}

function checkAndShowVoteStatus() {
    const currentStatus = localStorage.getItem('voteStatus') || 'open';
    if (currentStatus === 'closed') showVoteClosedBanner();
    else hideVoteClosedBanner();
}

// ========== GOUTTES DE CHAMPAGNE ==========
function lancerGouttesChampagne() {
    const container = document.getElementById('dropletsContainer');
    if (!container) return;
    const nbGouttes = 4 + Math.floor(Math.random() * 4);
    for(let i = 0; i < nbGouttes; i++) {
        setTimeout(() => {
            const droplet = document.createElement('div');
            droplet.className = 'droplet';
            droplet.style.left = (20 + Math.random() * 60) + 'px';
            droplet.style.top = (5 + Math.random() * 20) + 'px';
...
function lancerFeuArtifice() {
    ...
    function lancerVague() {
        if (Date.now() - debut >= dureeTotale) return;
        for(let i = 0; i < 30; i++) {
            const particle = document.createElement('div');
            particle.style.position = 'fixed';
            particle.style.width = '12px';
            particle.style.height = '12px';
            const colors = ['#D4AF37', '#E5C55E', '#FFD700', '#FFFFFF', '#B8960C'];
            ...
            setTimeout(() => {
                particle.style.transform = `translateY(${-150 - Math.random() * 250}px) translateX(${-60 + Math.random() * 120}px)`;
                particle.style.opacity = '0';
            }, 10);
            setTimeout(() => particle.remove(), 3000);
        }
        setTimeout(lancerVague, 200);
    }
    lancerVague();
    setTimeout(() => {
        for(let i = 0; i < 60; i++) {
            ...
            particle.style.backgroundColor = `hsl(${45 + Math.random() * 20}, 100%, 58%)`;
            particle.style.borderRadius = '50%';
            particle.style.left = Math.random() * window.innerWidth + 'px';
            particle.style.bottom = '0px';
            particle.style.pointerEvents = 'none';
            ...
            document.body.appendChild(particle);
            setTimeout(() => {
                particle.style.transform = `translateY(${-200 - Math.random() * 300}px) translateX(${-80 + Math.random() * 160}px)`;
                particle.style.opacity = '0';
            }, 10);
            setTimeout(() => particle.remove(), 3000);
        }
...
function lancerFeuArtificeResultats() {
    for(let i = 0; i < 100; i++) {
        setTimeout(() => {
            ...
            const colors = ['#D4AF37', '#E5C55E', '#FFD700', '#FFFFFF', '#B8960C', '#F5E056'];
            particle.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)];
            particle.style.borderRadius = '50%';
            particle.style.left = Math.random() * window.innerWidth + 'px';
            particle.style.bottom = '0px';
            ...
            setTimeout(() => {
                particle.style.transform = `translateY(${-distance}px) translateX(${angle}px)`;
                particle.style.opacity = '0';
            }, 10);
            setTimeout(() => particle.remove(), 3000);
        }, i * 30);
...
function selectionnerCategorie(categoryId) {
    ...
    liste.forEach(candidat => {
        ...
        let photoHtml = '';
        if (candidat.photo) {
            photoHtml = `<div class="candidate-photo" data-nom="${candidat.nom}" data-photo="${candidat.photo}"><img src="${candidat.photo}" alt="${candidat.nom}" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'no-photo\'><i class=\'fas fa-user-circle\'></i></div>'; this.parentElement.style.cursor='pointer';"></div>`;
        } else {
            photoHtml = `<div class="no-photo" data-nom="${candidat.nom}"><i class="fas fa-user-circle"></i></div>`;
        }
    ...
    document.querySelectorAll('.candidate-photo, .no-photo').forEach(photoDiv => {
        photoDiv.style.cursor = 'pointer';
        photoDiv.addEventListener('click', (e) => {
            e.stopPropagation();
            const nom = photoDiv.dataset.nom || 'Candidat';
            let photoUrl = photoDiv.dataset.photo;
            if (photoDiv.classList.contains('no-photo') || !photoUrl) openPhotoModal(null, nom);
            else openPhotoModal(photoUrl, nom);
        });
    });
    document.querySelectorAll('.vote-btn:not([disabled])').forEach(btn => {
        btn.onclick = () => voter(btn.dataset.categorie, btn.dataset.candidatNom);
    });
}
...
function voter(categorie, candidatNom) {
    ...
    if (currentStatus !== 'open') {
        if (currentStatus === 'paused') showToast("⏸️ Les votes sont temporairement suspendus.", "error");
        else showVoteClosedWarning();
        return;
    }
...
function afficherDashboard() {
    ...
    if (userVotes.length === 0) { 
        dashboardDiv.innerHTML = '<div class="loading-results"><i class="fas fa-spinner fa-pulse"></i> Vous n\'avez pas encore voté. Choisissez une catégorie ci-dessus !</div>'; 
        return; 
    }
    
    // Organiser par catégorie (seulement ses votes)
    const resultatsParCategorie = {};
    categories.forEach(cat => { 
        resultatsParCategorie[cat.id] = { 
            nom: cat.name, 
            monVote: null, 
            aVote: false 
        }; 
    ...
    for (const [catId, data] of Object.entries(resultatsParCategorie)) {
        const card = document.createElement('div');
        ...
        if (data.aVote) {
            ...
            card.innerHTML = `
                ...
                <div class="my-vote">
                    <i class="fas fa-vote-yea"></i> Votre vote : <strong>${data.monVote}</strong>
                </div>
            ...
            card.innerHTML = `
                ...
                <div class="my-vote">
                    <i class="fas fa-clock"></i> En attente de votre vote
                </div>
                <div class="vote-message">
                    Retournez voter pour cette catégorie
                </div>
            `;
        }
...
function markCategoryResultsAsDisplayed(categoryId) {
    let displayedCategories = JSON.parse(localStorage.getItem('displayedResultCategories') || '[]');
    if (!displayedCategories.includes(categoryId)) {
        displayedCategories.push(categoryId);
        localStorage.setItem('displayedResultCategories', JSON.stringify(displayedCategories));
    }
}

function isCategoryResultsDisplayed(categoryId) {
    let displayedCategories = JSON.parse(localStorage.getItem('displayedResultCategories') || '[]');
    return displayedCategories.includes(categoryId);
}

function clearDisplayedCategories() {
    localStorage.removeItem('displayedResultCategories');
}

// ========== TEMPS RÉEL POUR RÉSULTATS ==========
function startRealtimeResultsListener() {
    if (realtimeListener) return;
    
    realtimeListener = setInterval(() => {
        const currentPage = window.location.pathname;
        if (!currentPage.includes('resultat.html')) return;
        ...
        const displayedCats = JSON.parse(localStorage.getItem('displayedResultCategories') || '[]');
        ...
        if (pendingCategory) {
            ...
            if (timerEnd && parseInt(timerEnd) > Date.now()) {
                ...
                showCategoryCountdown(pendingCategory, remaining);
            } else {
                localStorage.removeItem('pendingResultCategory');
                localStorage.removeItem('pendingCategoryTimerEnd');
                const categoryToShow = pendingCategory;
                markCategoryResultsAsDisplayed(categoryToShow);
                const resultsMessage = document.getElementById('resultsMessage');
                const finalContainer = document.getElementById('finalResultsContainer');
                if (resultsMessage && finalContainer) {
                    resultsMessage.style.display = 'none';
                    finalContainer.style.display = 'block';
                    afficherResultatsParCategorie(categoryToShow);
        ...
        const notDisplayed = publishedCats.filter(cat => !displayedCats.includes(cat));
        
        if (notDisplayed.length > 0 && !allPublished) {
            const categoryToShow = notDisplayed[0];
            localStorage.setItem('pendingResultCategory', categoryToShow);
            localStorage.setItem('pendingCategoryTimerEnd', Date.now() + 10000);
            showCategoryCountdown(categoryToShow, 10);
        }
...
function showCategoryCountdown(categoryId, remaining) {
    ...
    if (!timerElement) {
        ...
        timerElement.id = 'resultsCountdownTimer';
        timerElement.className = 'results-countdown-timer';
    ...
    timerElement.innerHTML = `
        <div class="countdown-container">
            <div class="countdown-header">
                ...
                <div class="countdown-title">ANNONCE DES RÉSULTATS</div>
                <div class="countdown-category">${cat.icon} ${cat.name}</div>
            </div>
            <div class="countdown-timer-wrapper">
                <div class="countdown-circle">
                    <svg class="countdown-svg" viewBox="0 0 120 120">
                        <circle cx="60" cy="60" r="54" class="countdown-bg-circle"/>
                        <circle cx="60" cy="60" r="54" class="countdown-progress-circle" 
                                stroke-dasharray="339.292" stroke-dashoffset="0"
                                transform="rotate(-90 60 60)"/>
                    </svg>
                    <div class="countdown-number">
                        <span class="countdown-value" id="countdownValue">${remaining}</span>
                        <span class="countdown-unit">SEC</span>
                    </div>
                </div>
                <div class="countdown-progress-bar-wrapper">
                    <div class="countdown-progress-bar" id="countdownProgressFill" style="width: ${initialPercent}%"></div>
                </div>
            </div>
            <div class="countdown-footer">
                <div class="countdown-message">
                    <i class="fas fa-hourglass-half"></i> Préparation en cours...
                </div>
                <div class="countdown-suspense">
                    <span class="suspense-dot"></span>
                    <span class="suspense-dot"></span>
                    <span class="suspense-dot"></span>
                </div>
            </div>
    ...
    if (circle) {
        const initialOffset = circumference * (1 - (currentRemaining / totalSeconds));
    ...
    const interval = setInterval(() => {
        ...
        const percent = (currentRemaining / totalSeconds) * 100;
        if (progressFill) progressFill.style.width = `${percent}%`;
        
        if (circle) {
            const offset = circumference * (1 - (currentRemaining / totalSeconds));
        ...
        if (currentRemaining <= 0) {
            ...
            setTimeout(() => {
                ...
                localStorage.removeItem('pendingResultCategory');
                localStorage.removeItem('pendingCategoryTimerEnd');
                markCategoryResultsAsDisplayed(categoryId);
                ...
                if (resultsMessage && finalContainer) {
                    resultsMessage.style.display = 'none';
                    finalContainer.style.display = 'block';
                    afficherResultatsParCategorie(categoryId);
                    lancerFeuArtificeResultats();
                }
            }, 500);
        }
...
function initResultsPage() {
    ...
    if (lastPublishedCategory && !allPublished && publishedCats.length > 0) {
        ...
        const remaining = 10;
        showCategoryCountdown(lastPublishedCategory, remaining);
        return;
    }
...
function afficherResultatsParCategorie(categoryId) {
    ...
    if (winnerNom && !isTie) {
        const categoryCandidates = candidats[categoryId] || [];
        const winnerCandidate = categoryCandidates.find(c => c.nom === winnerNom);
        if (winnerCandidate && winnerCandidate.photo) {
            winnerPhotoUrl = winnerCandidate.photo;
        }
    }
    
    if (winnerPhotoContainer && winnerNom && !isTie) {
        winnerPhotoContainer.style.display = 'flex';
        const winnerPhotoDiv = document.getElementById('winnerPhoto');
        const winnerPhotoNameSpan = document.getElementById('winnerPhotoName');
        if (winnerPhotoUrl) {
            winnerPhotoDiv.innerHTML = `<img src="${winnerPhotoUrl}" alt="${winnerNom}" onclick="openPhotoModal('${winnerPhotoUrl}', '${winnerNom}')">`;
        } else {
            winnerPhotoDiv.innerHTML = `<div class="no-photo-result" onclick="openPhotoModal(null, '${winnerNom}')"><i class="fas fa-user-circle fa-5x"></i></div>`;
        }
        winnerPhotoNameSpan.innerHTML = `<i class="fas fa-crown"></i> ${winnerNom}`;
    } else if (winnerPhotoContainer) {
        winnerPhotoContainer.style.display = 'none';
    }
    
    grid.innerHTML = `
        <div class="final-result-card winner-card" style="grid-column:1/-1; text-align:center; margin-bottom:20px;">
            <div class="final-category-icon" style="font-size:3rem;">${cat.icon}</div>
            <h2 style="color:var(--gold); font-size:2rem;">${cat.name}</h2>
            <div class="winner-medal" style="font-size:2.5rem;">${winnerMedal}</div>
            <div class="winner-name" style="font-size:1.8rem;">${winnerDisplay}</div>
            <div class="winner-stats">${winnerVotesDisplay}${winnerPercent ? ` <i class="fas fa-chart-line"></i> (${winnerPercent}%)` : ''}</div>
            ${!isTie && second ? `<div class="second-place">${secondMedal} ${second[0]} <span class="candidate-stats">(${second[1]} voix)</span></div>` : ''}
            ${!isTie && third ? `<div class="third-place">${thirdMedal} ${third[0]} <span class="candidate-stats">(${third[1]} voix)</span></div>` : ''}
            <div class="total-votes" style="margin-top:15px;"><i class="fas fa-users"></i> Total: ${total} votants</div>
        </div>
    `;
    
    const congrats = document.createElement('div');
    congrats.className = 'congrats-banner';
    congrats.innerHTML = `<p><i class="fas fa-heart"></i> Félicitations à ${isTie ? 'nos ex-aequo' : (winner ? winner[0] : 'tous')} pour ce prix ! <i class="fas fa-heart"></i></p>`;
...
function afficherTousResultatsFinaux() {
    ...
    if (categoriesAvecVotes.length === 0) {
        grid.innerHTML = '<div class="empty-comments"><i class="fas fa-chart-simple"></i><p>Aucun vote enregistré pour le moment.</p></div>';
        return;
    }
    ...
    for (const [catId, data] of categoriesAvecVotes) {
        ...
        if (winnerNom && !isTie) {
            const categoryCandidates = candidats[catId] || [];
            const winnerCandidate = categoryCandidates.find(c => c.nom === winnerNom);
            if (winnerCandidate && winnerCandidate.photo) {
                winnerPhotoUrl = winnerCandidate.photo;
            }
        ...
        const photoHtml = winnerPhotoUrl ? 
            `<div class="winner-card-photo" onclick="openPhotoModal('${winnerPhotoUrl}', '${winnerNom}')">
                <img src="${winnerPhotoUrl}" alt="${winnerNom}">
             </div>` : 
            `<div class="winner-card-photo no-photo" onclick="openPhotoModal(null, '${winnerNom || ''}')">
                <i class="fas fa-user-circle"></i>
             </div>`;
        
        card.innerHTML = `
            <div class="final-category-icon">${data.icon}</div>
            <h3>${data.nom}</h3>
            ${!isTie ? photoHtml : '<div class="winner-card-photo no-photo"><i class="fas fa-handshake"></i></div>'}
            <div class="winner-medal">${winnerMedal}</div>
            <div class="winner-name">${winnerDisplay}</div>
            <div class="winner-stats"><i class="fas fa-chart-simple"></i> ${winner ? winner[1] + ' voix (' + winnerPercent + '%)' : '<i class="fas fa-ban"></i> Aucun vote'}</div>
            ${!isTie && second ? `<div class="second-place"><i class="fas fa-medal"></i> ${second[0]} <span class="candidate-stats">(${second[1]} voix)</span></div>` : ''}
            ${!isTie && third ? `<div class="third-place"><i class="fas fa-medal"></i> ${third[0]} <span class="candidate-stats">(${third[1]} voix)</span></div>` : ''}
            ${isTie ? `<div class="second-place"><i class="fas fa-handshake"></i> Égalité parfaite !</div>` : ''}
            <div class="total-votes" style="margin-top:15px;"><i class="fas fa-users"></i> Total: ${total} votants</div>
        `;
        grid.appendChild(card);
...
function updateVoteStatusUI() {
    ...
    switch(currentStatus) {
        case 'open': statusBadge.className = 'status-badge open'; statusBadge.innerHTML = '🟢 Votes ouverts'; if(statusText) statusText.innerHTML = 'Les utilisateurs peuvent voter normalement'; break;
        case 'paused': statusBadge.className = 'status-badge paused'; statusBadge.innerHTML = '🟡 Votes en pause'; if(statusText) statusText.innerHTML = 'Les utilisateurs voient un message d\'attente'; break;
        case 'closed': statusBadge.className = 'status-badge closed'; statusBadge.innerHTML = '🔴 Votes clôturés'; if(statusText) statusText.innerHTML = 'Les votes sont terminés. Prêt à publier.'; break;
        case 'published': statusBadge.className = 'status-badge published'; statusBadge.innerHTML = '📢 Résultats publiés'; if(statusText) statusText.innerHTML = 'Les résultats ont été publiés !'; break;
    }
}

function setVoteStatus(newStatus) {
    localStorage.setItem('voteStatus', newStatus);
    updateVoteStatusUI();
    const messages = { 'open': '✅ Votes ouverts.', 'paused': '⏸️ Votes en pause.', 'closed': '🔴 Votes clôturés.', 'published': '📢 Résultats publiés !' };
    showToast(messages[newStatus], "info");
    if (newStatus === 'closed') showVoteClosedBanner();
    else hideVoteClosedBanner();
}

// ========== PUBLICATION PAR CATÉGORIE AVEC TIMER ==========
function showPublicationTimer(categoryId) {
    ...
    timerModal.innerHTML = `
        <div class="publication-timer-content">
            <div class="publication-timer-header">
                <i class="fas fa-champagne-glasses"></i>
                <h3>Publication des résultats</h3>
            </div>
            <div class="publication-timer-body">
                <p>Préparez-vous à publier les résultats de :</p>
                <h2>${cat.icon} ${cat.name}</h2>
                <div class="publication-timer-circle">
                    <span class="publication-timer-number" id="publicationTimerNumber">10</span>
                    <span class="publication-timer-label">secondes</span>
                </div>
                <div class="publication-timer-progress">
                    <div class="publication-timer-progress-bar" id="publicationTimerProgress"></div>
                </div>
                <p class="publication-timer-warning">⚠️ La publication est imminente</p>
            </div>
            <div class="publication-timer-footer">
                <button id="cancelPublicationBtn" class="publication-cancel-btn">
                    <i class="fas fa-times"></i> Annuler
                </button>
                <button id="confirmPublicationBtn" class="publication-confirm-btn">
    ...
    publicationTimerInterval = setInterval(() => {
        countdown--;
        if (timerNumber) timerNumber.textContent = countdown;
        if (progressBar) progressBar.style.width = `${(10 - countdown) * 10}%`;
        
        if (countdown <= 0) {
            clearInterval(publicationTimerInterval);
            publicationTimerInterval = null;
            closePublicationTimerModal();
            executePublishCategory(categoryId);
        }
    }, 1000);
    
    const cancelBtn = document.getElementById('cancelPublicationBtn');
    if (cancelBtn) {
        cancelBtn.onclick = () => {
            clearInterval(publicationTimerInterval);
            publicationTimerInterval = null;
    ...
    const confirmBtn = document.getElementById('confirmPublicationBtn');
    if (confirmBtn) {
        confirmBtn.onclick = () => {
            clearInterval(publicationTimerInterval);
            publicationTimerInterval = null;
...
function closePublicationTimerModal() {
    const modal = document.getElementById('publicationTimerModal');
    if (modal) {
...
function executePublishCategory(categoryId) {
    publishedCategories.push(categoryId);
    savePublishedCategories();
    localStorage.setItem('lastPublishedCategory', categoryId);
    localStorage.setItem('resultsDisplayed', 'false');
    ...
    showToast(`✅ Résultats de la catégorie publiés !`, "success");
}

function loadPublishedCategories() {
    publishedCategories = JSON.parse(localStorage.getItem('publishedCategories') || '[]');
    allResultsPublished = localStorage.getItem('allResultsPublished') === 'true';
}

function savePublishedCategories() {
    localStorage.setItem('publishedCategories', JSON.stringify(publishedCategories));
    localStorage.setItem('allResultsPublished', allResultsPublished ? 'true' : 'false');
}
...
function publishCategory(categoryId) {
    ...
    showPublicationTimer(categoryId);
}

function publishAllCategories() {
    if (allResultsPublished) { showToast("Tous les résultats ont déjà été publiés.", "error"); return; }
    const currentStatus = localStorage.getItem('voteStatus') || 'open';
    if (currentStatus !== 'closed') { showToast("Veuillez d'abord clôturer les votes.", "error"); return; }
    
    const unpublished = categories.filter(cat => !isCategoryPublished(cat.id));
    ...
    if (noVotesCats.length > 0) {
        showToast(`❌ Publication impossible : ${noVotesCats.join(', ')} n'ont aucun vote.`, "error");
        return;
    }
    
    unpublished.forEach(cat => { if (!publishedCategories.includes(cat.id)) publishedCategories.push(cat.id); });
    allResultsPublished = true;
    savePublishedCategories();
    localStorage.setItem('voteStatus', 'published');
    localStorage.setItem('resultsPublished', 'true');
    localStorage.setItem('allResultsPublished', 'true');
...
function renderPublishCategoriesGrid() {
    ...
    const allCategoriesHaveVotes = categories.every(cat => {
        const votes = votesByCategory[cat.id];
        const total = Object.entries(votes).reduce((sum, [, nb]) => sum + nb, 0);
        return total > 0;
    });
    
    const allPublishedNow = categories.every(cat => isCategoryPublished(cat.id));
    
    categories.forEach(cat => {
        const isPublished = isCategoryPublished(cat.id);
        const votes = votesByCategory[cat.id];
        const sorted = Object.entries(votes).sort((a, b) => b[1] - a[1]);
        const winner = sorted[0];
        const total = sorted.reduce((sum, [, nb]) => sum + nb, 0);
        const hasVotes = total > 0;
        const isDisabled = currentStatus !== 'closed' || !hasVotes;
        
        const card = document.createElement('div');
        card.className = `publish-category-card ${isPublished ? 'published' : ''} ${isDisabled ? 'disabled' : ''}`;
        
        let winnerHtml = '';
        if (!hasVotes) {
            winnerHtml = '❌ Aucun vote - Publication impossible';
        } else if (isPublished) {
            winnerHtml = `✅ Publié : ${winner ? winner[0] : '-'} (${winner[1]} voix)`;
        } else {
            winnerHtml = winner ? `🏆 ${winner[0]}<br><small>${winner[1]} voix</small>` : '⏳ Aucun vote';
        }
        
        card.innerHTML = `<div class="publish-category-icon">${cat.icon}</div><h4>${cat.name}</h4><div class="publish-category-stats">${total} vote${total !== 1 ? 's' : ''}</div><div class="publish-category-winner">${winnerHtml}</div>`;
        
        if (!isPublished && !isDisabled && hasVotes) card.onclick = () => publishCategory(cat.id);
        container.appendChild(card);
    });
    
    const publishAllBtn = document.getElementById('publishAllResultsBtn');
    if (publishAllBtn) {
        const allHaveVotes = allCategoriesHaveVotes;
        const allPublished = allPublishedNow;
        const canPublishAll = currentStatus === 'closed' && allHaveVotes && !allPublished;
        
        if (canPublishAll) {
            publishAllBtn.disabled = false;
            publishAllBtn.style.opacity = '1';
            publishAllBtn.style.cursor = 'pointer';
            publishAllBtn.innerHTML = '<i class="fas fa-fire"></i> 🎉 PUBLIER TOUS LES RÉSULTATS 🎉';
        } else if (allPublished) {
            publishAllBtn.disabled = true;
            publishAllBtn.style.opacity = '0.5';
            publishAllBtn.style.cursor = 'not-allowed';
            publishAllBtn.innerHTML = '<i class="fas fa-check-circle"></i> ✅ TOUS LES RÉSULTATS SONT PUBLIÉS';
        } else {
            publishAllBtn.disabled = true;
            publishAllBtn.style.opacity = '0.5';
            publishAllBtn.style.cursor = 'not-allowed';
            if (!allHaveVotes) {
                publishAllBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> ⚠️ ATTENTION : Certaines catégories n\'ont aucun vote';
            } else {
                publishAllBtn.innerHTML = '<i class="fas fa-lock"></i> 🔒 Clôturez d\'abord les votes';
            }
        }
    }
}

// ========== GRAPHIQUE AVEC ANIMATION PROGRESSIVE ==========
function animateChartData(newData) {
    if (!votesChart) return;
    
    const steps = 20;
    const stepDuration = 30;
    const startData = votesChart.data.datasets[0].data;
    const diff = newData.map((val, idx) => val - startData[idx]);
    
    let step = 0;
    const interval = setInterval(() => {
        step++;
        const progress = step / steps;
        const intermediateData = startData.map((val, idx) => val + diff[idx] * progress);
        votesChart.data.datasets[0].data = intermediateData;
        votesChart.update('none');
        if (step >= steps) {
            clearInterval(interval);
            votesChart.data.datasets[0].data = newData;
            votesChart.update();
        }
    }, stepDuration);
}
...
function initVotesChart() {
    ...
    const votesData = categories.map(cat => votesByCategory[cat.id]);
    ...
    votesChart = new Chart(ctx, {
        ...
        options: { 
            ...
            scales: {
                ...
                x: { grid: { display: false }, ticks: { color: '#b0b0b0', rotation: 45, maxRotation: 45 } }
            },
            plugins: { 
                legend: { labels: { color: '#b0b0b0', font: { family: 'Montserrat' } } }, 
                tooltip: { 
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            const percentage = ((value / MAX_VOTES) * 100).toFixed(1);
                            return `${value} voix (${percentage}% du maximum ${MAX_VOTES})`;
                        }
                    }
                } 
            }
...
function initTableFilters() {
    ...
    if (filterCategory) filterCategory.addEventListener('input', () => applyTableFilters());
    if (filterCandidate) filterCandidate.addEventListener('input', () => applyTableFilters());
}

function applyTableFilters() {
    const filterCategory = document.getElementById('filterCategory')?.value.toLowerCase() || '';
    const filterCandidate = document.getElementById('filterCandidate')?.value.toLowerCase() || '';
    const tbody = document.getElementById('votesTableBody');
    if (!tbody) return;
    const rows = tbody.querySelectorAll('tr');
    rows.forEach(row => {
        const category = row.cells[1]?.textContent.toLowerCase() || '';
        const candidate = row.cells[2]?.textContent.toLowerCase() || '';
        row.style.display = (filterCategory === '' || category.includes(filterCategory)) && (filterCandidate === '' || candidate.includes(filterCandidate)) ? '' : 'none';
    });
}

function checkAdminSession() {
    if (localStorage.getItem('adminLoggedIn') === 'true' && document.getElementById('adminZone')) {
...
function initAdmin() {
    ...
    if (loginBtn) {
        loginBtn.onclick = () => {
            if (adminCode.value === "GALA2026" || adminCode.value === "admin123") {
...
async function chargerAdminData() {
    ...
    const votesRestants = Math.max(0, MAX_VOTES - totalVotes);
    
    const statsGrid = document.getElementById('statsGrid');
    if(statsGrid) statsGrid.innerHTML = `
        <div class="stat-card"><div class="stat-number">${totalVotes}</div><div class="stat-label">Votes totaux</div></div>
        <div class="stat-card"><div class="stat-number">${totalVotants}</div><div class="stat-label">Votants uniques</div></div>
        <div class="stat-card"><div class="stat-number">${categories.length}</div><div class="stat-label">Catégories</div></div>
        <div class="stat-card"><div class="stat-number">${tauxParticipation}%</div><div class="stat-label">Objectif atteint</div></div>
        <div class="stat-card"><div class="stat-number">${votesRestants}</div><div class="stat-label">Votes restants</div></div>
        <div class="stat-card"><div class="stat-number">${MAX_VOTES}</div><div class="stat-label">Objectif max</div></div>
    `;
    ...
    if(container) {
        ...
        for (const [catId, data] of Object.entries(resultatsParCategorie)) {
            const sorted = Object.entries(data.votes).sort((a, b) => b[1] - a[1]);
            const winner = sorted[0];
            const catIcon = categories.find(c => c.id === catId)?.icon || '';
            const card = document.createElement('div');
            card.className = 'category-summary-card';
            card.innerHTML = `
                <h4>${catIcon} ${data.nom}</h4>
                <div class="total"><i class="fas fa-chart-simple"></i> Total votes : ${data.total}</div>
                <div class="winner"><i class="fas fa-crown"></i> Leader : ${winner ? winner[0] : 'Aucun vote'} (${winner ? winner[1] : 0} voix)</div>
            `;
            container.appendChild(card);
        }
    }
    ...
    if(tbody) {
        if(localVotes.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="loading-cell">Aucun vote enregistré</td></tr>';
        } else {
            tbody.innerHTML = '';
...
function exporterDonneesAdmin() {
    ...
    const data = { 
        ...
        statistiques: { 
            ...
            totalVotants: Object.keys(localVotants).length, 
            categories: categories.length, 
            voteStatus: localStorage.getItem('voteStatus'), 
            maxVotes: MAX_VOTES 
        }, 
        votes: localVotes, 
        votants: localVotants, 
    ...
    a.download = `votes_gala_${new Date().toISOString().slice(0,19)}.json`;
    a.click();
...
function openPhotoModal(photoUrl, nom) {
    ...
    if (photoUrl) {
        ...
        modalImg.alt = nom;
        modalCaption.innerHTML = `<i class="fas fa-user"></i> ${nom}`;
        modalImg.style.display = 'block';
    } else {
        modalImg.style.display = 'none';
        modalCaption.innerHTML = `<i class="fas fa-user-circle fa-5x"></i><br><br>${nom}<br><span style="font-size:0.8rem;">Photo non disponible</span>`;
    }
...
function addForceRefreshButton() {
    ...
    btn.innerHTML = '<i class="fas fa-sync-alt"></i> Voir les résultats publiés';
    btn.style.cssText = `
        position: fixed;
        bottom: 20px;
        right: 20px;
    ...
    btn.onmouseover = () => {
        ...
        btn.style.boxShadow = '0 4px 15px rgba(212,175,55,0.5)';
    };
    btn.onmouseout = () => {
    ...
    btn.onclick = () => {
        ...
        showToast("Résultats rechargés !", "success");
    };
